...
## 2025-06-25 - [Tactile and Accessible Selection States]
**Learning:** For selection-based interfaces like the Feedback category picker, simple visual changes (like background color) are insufficient for screen readers and lack "heft" on mobile.
**Action:** Always use `aria-pressed` for toggleable/selectable buttons and combine with `triggerHaptic()` and `active:scale-95` to provide a multi-modal confirmation of the user's action.
