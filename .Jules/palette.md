...
## 2025-01-24 - [Mobile-First Tactile Feedback]
**Learning:** In a Capacitor-wrapped web application, "native" feel is significantly enhanced by combining haptic feedback (`triggerHaptic`) with subtle visual scaling (`active:scale-95`). This standardized pairing provides immediate, multi-sensory confirmation of interactions, bridging the gap between web and native UX.
**Action:** Standardize tactile feedback for all primary interactive elements by applying `active:scale-95` and `triggerHaptic()` together.
