...
## 2025-07-24 - [Tactile Feedback for Mobile Interactions]
**Learning:** Standard buttons on mobile can feel unresponsive without tactile feedback. Combining CSS scaling (`active:scale-95`) with Capacitor haptics (`triggerHaptic`) provides a significantly more "native" and satisfying feel for interactive elements.
**Action:** Apply `active:scale-95` and `triggerHaptic()` to all interactive icon-only buttons to enhance perceived responsiveness on mobile.
