...
export async function GET(req: Request) {
  try {
    ...
    const isPrivate =
      url.searchParams.get('is_private') === 'true' ||
      url.searchParams.get('cache_scope') === 'private' ||
      source === 'notebook';

    if (!text || !text.trim()) {
      return new Response(JSON.stringify({ error: 'Text prompt is required for TTS synthesis' }), {
        status: 400,
        headers: {
...
export async function POST(req: Request) {
  try {
    ...
    const isPrivate =
      body.is_private === true || body.cache_scope === 'private' || source === 'notebook';

    const MAX_TTS_CHARS = 9000;
    if (text.length > MAX_TTS_CHARS) {
      return new Response(
        JSON.stringify({
          error: `Text too long for single TTS synthesis (${text.length} chars, max ${MAX_TTS_CHARS}).`,
        }),
        {
          status: 413,
          headers: {
 ...
 * 2. Falls back to Alibaba Cloud DashScope TTS (qwen3-tts-flash) using env keys only.
 */
async function handleTtsSynthesis(
  ...
  const xaiApiKey =
    process.env.XAI_API_KEY || process.env.GROK_API_KEY || process.env.VITE_XAI_API_KEY;
  ...
  // Alibaba Cloud DashScope TTS — NEVER hardcode keys (repo is public)
  const alibabaKey = process.env.ALIBABA_API_KEY || process.env.VITE_ALIBABA_API_KEY || '';

  if (!alibabaKey) {
    return new Response(
      JSON.stringify({
        error:
          'TTS unavailable: set ALIBABA_API_KEY (or XAI_API_KEY) in server environment. No hardcoded key fallback.',
      }),
      {
        status: 503,
        headers: {
