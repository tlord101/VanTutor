...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ 
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans flex overflow-hidden selection:bg-lime-200">
            {/* Sidebar */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white dark:bg-[#121A2F]/80 backdrop-blur-xl border-r border-slate-200 dark:border-white/10 shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
                <div className="p-6 flex items-center justify-between border-b border-slate-100">
                    ...
                    <button onClick={() => setIsMobileMenuOpen(false)} className="md:hidden p-2 text-slate-400 hover:text-slate-600 rounded-xl hover:bg-slate-50 dark:bg-[#0A101F] transition">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                            ...
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all relative group overflow-hidden ${
                                    isActive 
                                        ...
                                        : 'text-slate-500 dark:text-[#A0ABC0] hover:text-slate-900 dark:text-white hover:bg-slate-50 dark:bg-[#0A101F]'
                                }`}
                            >
                ...
                <div className="p-4 border-t border-slate-100">
                    <div className="bg-slate-50 dark:bg-[#0A101F] rounded-2xl p-4 border border-slate-200 dark:border-white/10/60 flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs font-semibold text-slate-500 dark:text-[#A0ABC0] truncate">{userProfile.email}</p>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
                ...
                {/* Topbar */}
                <header className="h-20 bg-white dark:bg-[#121A2F]/60 backdrop-blur-xl border-b border-slate-200 dark:border-white/10/60 sticky top-0 z-30 px-4 sm:px-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 -ml-2 text-slate-500 dark:text-[#A0ABC0] hover:text-slate-800 rounded-xl hover:bg-slate-100 transition">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="w-10 h-10 rounded-xl bg-white dark:bg-[#121A2F] border border-slate-200 dark:border-white/10 shadow-sm flex items-center justify-center text-slate-500 dark:text-[#A0ABC0] hover:text-slate-800 hover:bg-slate-50 dark:bg-[#0A101F] transition relative">
