...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ 
    ...
    return (
        <div className="min-h-screen bg-[#f8fafc] text-slate-800 font-sans flex overflow-hidden selection:bg-lime-200">
            {/* Sidebar */}
            <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-white/80 backdrop-blur-xl border-r border-slate-200 shadow-2xl transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] flex flex-col ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
                <div className="p-6 flex items-center justify-between border-b border-slate-100">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="font-black text-xl tracking-tight text-slate-900 leading-tight">Admin<span className="text-lime-600">Pro</span></h1>
            ...
            <main className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden relative">
                ...
                {/* Topbar */}
                <header className="h-20 bg-white/60 backdrop-blur-xl border-b border-slate-200/60 sticky top-0 z-30 px-4 sm:px-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <button onClick={() => setIsMobileMenuOpen(true)} className="md:hidden p-2 -ml-2 text-slate-500 hover:text-slate-800 rounded-xl hover:bg-slate-100 transition">
                    ...
                    <div className="flex items-center gap-3">
                        <button className="w-10 h-10 rounded-xl bg-white border border-slate-200 shadow-sm flex items-center justify-center text-slate-500 hover:text-slate-800 hover:bg-slate-50 transition relative">
