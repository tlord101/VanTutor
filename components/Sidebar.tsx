    ...
    <li className="relative">
        <button
            ...
            className={`w-full flex transition-all duration-350 ease-in-out group hover:scale-[1.02] ${
                ...
                : (isModal ? 'bg-white/50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:bg-blue-50 dark:hover:bg-slate-700' : 'text-slate-655 opacity-85 hover:bg-white/50 hover:text-blue-600 hover:opacity-100')
            }`}
        >
            {!isModal && isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 h-6 w-1 bg-white rounded-r-full"></div>}
            <span className={`flex-shrink-0 transition-all duration-300 ease-in-out ${!isModal && isExpanded ? 'mr-4' : 'mr-0'} ${!isModal && isActive ? 'text-white' : (isActive ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 group-hover:text-blue-600')}`}>{item.icon}</span>
...
const SidebarContent: React.FC<{
    ...
    <div className={`h-full p-4 flex flex-col ${isModal ? '' : 'bg-transparent'}`}>
      ...
      <div className="flex-shrink-0">
        <div className={`mt-6 p-3 rounded-xl border relative ${isModal ? 'bg-slate-50 dark:bg-slate-800/50 border-slate-100 dark:border-slate-700' : 'bg-white/45 backdrop-blur-sm border-white/50 shadow-sm'}`}>
          {unreadCount > 0 && (
          ...
          <div className="flex items-center">
            ...
            <div className={`ml-3 whitespace-nowrap overflow-hidden transition-opacity duration-300 ease-in-out ${isExpanded ? 'opacity-100' : 'opacity-0'}`}>
              ...
              <p className="text-[10px] text-slate-500 dark:text-slate-400 font-extrabold uppercase tracking-widest">{userProfile?.level} Level</p>
            </div>
          </div>
        </div>
      </div>
...
export const Sidebar: React.FC<SidebarProps> = ({ activeItem, onItemClick, userProfile, onLogout, isMobileSidebarOpen, onCloseMobileSidebar, items, secondaryItems, unreadCount = 0, unreadMessagesCount = 0 }) => {
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <aside 
          className={`hidden md:block flex-shrink-0 bg-white/60 dark:bg-card/40 backdrop-blur-lg border-r border-white/40 dark:border-border shadow-sm w-72 h-full`}
      >
        <SidebarContent 
            isExpanded={true}
            activeItem={activeItem}
