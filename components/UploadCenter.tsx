...
Return a JSON object with a 'courses' array, where each item has 'course_name' and 'course_code'.`;
  ...
  const renderAuth = () => (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(14,165,233,0.18),_transparent_40%),linear-gradient(180deg,_#f0f9ff_0%,_#fff_100%)] px-4 py-8 text-slate-900 dark:text-white">
      <div className="mx-auto flex min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center justify-center">
        <div className="grid w-full gap-8 rounded-[32px] border border-sky-100 bg-white dark:bg-black/90 p-6 shadow-xl backdrop-blur md:grid-cols-[1.1fr_0.9fr] md:p-8">
  ...
  if (isAuthLoading || (user && isProfileLoading)) {
    return (
      <div className="flex min-h-screen bg-slate-50 dark:bg-black p-6">
        <PageSkeleton />
      </div>
    );
  }
  ...
  if (isCatalogLoading && Object.keys(schoolsData || {}).length === 0) {
      return (
        <div className="flex min-h-screen bg-slate-50 dark:bg-black p-6 flex-1 w-full lg:pl-64">
           <PageSkeleton />
        </div>
      );
  }
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-black text-slate-900 dark:text-white overflow-hidden relative">
      {/* Mobile Menu Overlay */}
      {isMobileMenuOpen && (
        <div 
      ...
      <aside className={`w-64 border-r border-slate-200 dark:border-white/10 bg-white dark:bg-black flex flex-col fixed inset-y-0 left-0 z-50 transition-transform duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0`}>
        <div className="p-6 flex items-center justify-between">
          ...
          <button className="md:hidden p-2 text-slate-400 hover:text-slate-700 bg-slate-50 dark:bg-black rounded-xl" onClick={() => setIsMobileMenuOpen(false)}>
            <X className="w-5 h-5" />
          </button>
        </div>
        <nav className="flex-1 px-4 space-y-2 overflow-y-auto">
          <button onClick={() => navigate('/upload-center')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${activeView === 'dashboard' ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50 dark:bg-black'}`}><LayoutDashboard className="w-5 h-5" /> Dashboard</button>
          <button onClick={() => navigate('/upload-center/courses')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${activeView === 'courses' ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50 dark:bg-black'}`}><BookOpen className="w-5 h-5" /> Course Directory</button>
          <button onClick={() => navigate('/upload-center/departments')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${activeView === 'departments' ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50 dark:bg-black'}`}><FolderOpen className="w-5 h-5" /> Departments</button>
          <button onClick={() => navigate('/upload-center/past-questions')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${activeView === 'past_questions' ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50 dark:bg-black'}`}><FileQuestion className="w-5 h-5" /> Past Questions</button>
          <button onClick={() => navigate('/upload-center/requests')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition ${activeView === 'requests' ? 'bg-sky-50 text-sky-700' : 'text-slate-600 hover:bg-slate-50 dark:bg-black'}`}><List className="w-5 h-5" /> All Requests</button>
        </nav>
        <div className="p-4 border-t border-slate-100 mt-auto">
          <button onClick={handleLogout} className="w-full py-2 text-sm font-bold text-slate-500 dark:text-gray-400 hover:text-slate-800 transition">Sign Out</button>
        </div>
      </aside>
              ...
              {uploads.length ? (
                <div className="space-y-3">
                  {uploads.slice(0, 5).map(up => (
                    <div key={up.course_key + up.uploaded_at} className="p-4 bg-slate-50 dark:bg-black rounded-2xl flex justify-between items-center">
                      <div>
                        <p className="font-bold">{up.course_name}</p>
                        <p className="text-sm text-slate-500 dark:text-gray-400">{up.level} - {up.semester}</p>
                      </div>
                      <span className="text-xs bg-green-100 text-green-700 px-3 py-1 rounded-full font-bold">Uploaded</span>
                    </div>
                  ))}
                </div>
              ) : <p className="text-slate-500 dark:text-gray-400 text-sm">No recent uploads.</p>}
            </div>
          </div>
        )}
                    ...
                    return (
                      <div key={course.course_id + course.level} className={`bg-white dark:bg-black border rounded-[24px] p-6 shadow-sm transition-all ${isUploaded ? 'border-green-200' : 'border-slate-200 dark:border-white/10'}`}>
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-1 rounded-full ${isUploaded ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500 dark:text-gray-400'}`}>{isUploaded ? 'Textbook Ready' : 'No Textbook'}</span>
                        </div>
            ...
            {selectedDepartmentId && (
              <div className="space-y-6 animate-fade-in">
                <div className="flex items-center gap-4">
                    <button onClick={() => setSelectedDepartmentId('')} className="p-2 bg-white dark:bg-black rounded-full shadow-sm hover:bg-slate-50 dark:bg-black transition">
                ...
                {isAddingCourse && (
                  <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm animate-fade-in">
                    <div className="w-full max-w-2xl rounded-[32px] border border-sky-100 bg-white dark:bg-black p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto">
                      ...
                      <p className="text-sm text-slate-500 dark:text-gray-400 font-medium mb-6">Add courses manually or extract them from a syllabus document.</p>
                      ...
                      {extractedCourses.length > 0 ? (
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="bg-indigo-50/50 border border-indigo-100 rounded-2xl p-5">
                            ...
                            <label className="flex items-center justify-center w-full py-3 bg-white dark:bg-black border border-indigo-200 text-indigo-600 rounded-xl font-bold hover:bg-indigo-50 transition cursor-pointer">
                              {isExtractingCourses ? (
          ...
          <div className="max-w-5xl space-y-6">
             ...
             <div className="bg-white dark:bg-black rounded-[24px] border border-slate-200 dark:border-white/10 shadow-sm p-6">
                {requests.length ? (
                  ...
                  <p className="text-slate-500 dark:text-gray-400">No requests submitted.</p>
                )}
             </div>
          </div>
        )}
      ...
      {uploadModal && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-[32px] border border-sky-100 bg-white dark:bg-black p-8 shadow-2xl space-y-6 relative">
            <h3 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Upload Material</h3>
            <p className="text-sm text-slate-500 dark:text-gray-400 font-medium">Select what type of material you are uploading for {uploadModal.course.course_name}.</p>
            ...
            >
                <div className="flex flex-col gap-3">
                    <div className="text-sm font-bold text-slate-500 dark:text-gray-400 mb-2 pointer-events-none">
                ...
                <button 
                  ...
                  className="w-full py-3 bg-[#4285F4] text-white rounded-xl font-bold hover:bg-[#3367D6] transition flex items-center justify-center gap-2">
                    <svg className="w-5 h-5 bg-white dark:bg-black rounded-full p-0.5" viewBox="0 0 24 24"><path fill="#FFC107" d="M16 14.5L12 21.5H5.333L9.333 14.5H16Z"/><path fill="#1976D2" d="M9.333 14.5L5.333 21.5L1.333 14.5L5.333 7.5L9.333 14.5Z"/><path fill="#4CAF50" d="M16 14.5H9.333L5.333 7.5H12L16 14.5Z"/><path fill="#000000" fillOpacity="0.2" d="M16 14.5L12 21.5H5.333L9.333 14.5H16Z"/><path fill="#000000" fillOpacity="0.2" d="M9.333 14.5L5.333 21.5L1.333 14.5L5.333 7.5L9.333 14.5Z"/><path fill="#000000" fillOpacity="0.2" d="M16 14.5H9.333L5.333 7.5H12L16 14.5Z"/></svg>
...
{/* Progress Modal */}
      {uploadProgress && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-slate-900/40 p-4 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md rounded-[32px] border border-sky-100 bg-white dark:bg-black p-8 shadow-2xl">
