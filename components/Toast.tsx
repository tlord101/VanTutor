...
export const Toast: React.FC<ToastProps> = ({ message, type, onDismiss }) => {
  ...
  return (
    <div
      className={`relative w-full bg-slate-800/60 backdrop-blur-sm text-white rounded-xl shadow-2xl flex items-start p-4 overflow-hidden border pointer-events-auto ${config.borderColorClass} ${isExiting ? 'animate-toast-out' : 'animate-toast-in'}`}
      role="alert"
    >
      ...
      <button
        ...
        className="ml-4 flex-shrink-0 p-1 rounded-full text-gray-400 hover:bg-white dark:bg-black/20 hover:text-white transition-colors"
        aria-label="Close"
      >
