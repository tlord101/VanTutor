...
export const LandingPage: React.FC<LandingPageProps> = ({ onLogin, onSignUp }) => {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] text-slate-900 dark:text-white font-sans overflow-x-hidden selection:bg-brand-500 selection:text-white">
      ...
      <div className="fixed inset-0 z-0 pointer-events-none overflow-hidden bg-white dark:bg-[#121A2F]">
      ...
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white dark:bg-[#121A2F]/80 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 py-4 shadow-sm' : 'bg-transparent py-6'}`}>
      ...
      <main className="relative z-10">
          ...
          <div className="max-w-4xl mx-auto flex flex-col items-center relative z-10 w-full text-center space-y-8">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4 justify-center pt-4">
              ...
              <button onClick={() => {
                onLogin();
                window.dispatchEvent(new Event('popstate'));
              }} className="w-full sm:w-auto px-8 py-3.5 bg-white dark:bg-[#121A2F] hover:bg-slate-50 dark:bg-[#0A101F] text-slate-700 rounded-lg font-semibold text-base transition border border-slate-300">
                Sign In to Dashboard
              </button>
            </div>
            <div className="flex items-center justify-center gap-4 pt-6 text-sm font-bold text-slate-500 dark:text-[#A0ABC0]">
              <div className="flex -space-x-3">
                ...
                <div className="w-8 h-8 rounded-full border-2 border-white bg-slate-50 dark:bg-[#0A101F]0 shadow-sm overflow-hidden"><img src="https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&w=100&q=80" alt="Student" className="w-full h-full object-cover" /></div>
              </div>
        ...
        {/* Mockup Section */}
        <section className="bg-white dark:bg-[#121A2F] py-12 px-6 flex justify-center relative z-10 w-full overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto bg-gradient-to-br from-brand-600 to-sky-500 rounded-[40px] p-12 md:p-20 text-center relative overflow-hidden shadow-2xl shadow-brand-500/20">
            ...
            <div className="relative z-10 space-y-8">
              ...
              <button onClick={() => {
                onSignUp();
                window.dispatchEvent(new Event('popstate'));
              }} className="px-10 py-5 bg-white dark:bg-[#121A2F] text-brand-700 rounded-2xl font-black text-xl hover:scale-105 transition duration-300 shadow-xl">
                Get Started Now
              </button>
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="border-t border-slate-200 dark:border-white/10 bg-white dark:bg-[#121A2F] py-12 px-6 relative z-10">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-8 text-sm font-bold text-slate-500 dark:text-[#A0ABC0]">
      ...
      <AnimatePresence>
        {showAppPopup && (
          <motion.div 
            ...
            className="fixed bottom-6 right-6 z-50 w-full max-w-sm bg-white dark:bg-[#121A2F] rounded-2xl shadow-2xl border border-slate-200 dark:border-white/10 overflow-hidden"
          >
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={handleAndroidDownload} className={`flex-1 text-sm font-semibold py-2 rounded-lg transition flex items-center justify-center gap-2 ${downloadUrl ? 'bg-slate-100 hover:bg-slate-200 text-slate-700' : 'bg-slate-50 dark:bg-[#0A101F] text-slate-400 cursor-not-allowed'}`}>
