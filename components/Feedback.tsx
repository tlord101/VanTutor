          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <button
                  type="button"
                  onClick={() => { setFeedbackType('suggestion'); void triggerHaptic(); }}
                  aria-pressed={feedbackType === 'suggestion'}
                  aria-label="Select suggestion feedback type"
                  className={`p-4 rounded-2xl border-2 text-left transition-all duration-200 active:scale-[0.98] ${feedbackType === 'suggestion' ? 'border-[#009EE2] bg-[#009EE2]/5 text-[#009EE2]' : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20'}`}
                >
                ...
                <button
                  type="button"
                  onClick={() => { setFeedbackType('bug'); void triggerHaptic(); }}
                  aria-pressed={feedbackType === 'bug'}
                  aria-label="Select bug report feedback type"
                  className={`p-4 rounded-2xl border-2 text-left transition-all duration-200 active:scale-[0.98] ${feedbackType === 'bug' ? 'border-red-500 bg-red-500/5 text-red-600' : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20'}`}
                >
                  <div className="flex items-center gap-2 mb-1">
                ...
                <button
                  type="button"
                  onClick={() => { setFeedbackType('complaint'); void triggerHaptic(); }}
                  aria-pressed={feedbackType === 'complaint'}
                  aria-label="Select complaint feedback type"
                  className={`p-4 rounded-2xl border-2 text-left transition-all duration-200 active:scale-[0.98] ${feedbackType === 'complaint' ? 'border-orange-500 bg-orange-500/5 text-orange-600' : 'border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 hover:border-slate-300 dark:hover:border-white/20'}`}
                >
                  <div className="flex items-center gap-2 mb-1">
