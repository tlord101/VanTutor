...
export const StudyPartners: React.FC<StudyPartnersProps> = ({ userProfile, onNavigate }) => {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#F8F9FA] animate-fade-in">
            {/* Header */}
            <div className="flex items-center px-4 py-4 bg-white border-b border-[#E9ECEF] shrink-0 sticky top-0 z-10">
                ...
                <div className="flex-1">
                    <h1 className="text-xl font-black text-[#212529]">Study Partners</h1>
                    <p className="text-xs font-semibold text-[#6C757D]">Build your academic network</p>
                </div>
            </div>

            {/* Tabs */}
            <div className="flex border-b border-[#E9ECEF] bg-white px-4 py-3 shrink-0 gap-3">
                <button
                    onClick={() => setActiveTab('find')}
                    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition ${activeTab === 'find' ? 'bg-[#009EE2] text-white shadow-md shadow-[#009EE2]/20' : 'text-[#6C757D] hover:text-[#212529] bg-neutral-50 hover:bg-neutral-100'}`}
                >
                    Find Partners
                </button>
                <button
                    onClick={() => setActiveTab('requests')}
                    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition flex items-center gap-2 ${activeTab === 'requests' ? 'bg-[#009EE2] text-white shadow-md shadow-[#009EE2]/20' : 'text-[#6C757D] hover:text-[#212529] bg-neutral-50 hover:bg-neutral-100'}`}
                >
                    Requests
                    {(receivedRequests.length > 0 || sentRequests.length > 0) && (
                        <span className={`rounded-full text-[10px] font-black h-5 px-1.5 flex items-center justify-center ${activeTab === 'requests' ? 'bg-white text-[#009EE2]' : 'bg-red-500 text-white'}`}>
            ...
            <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6">
                {activeTab === 'find' ? (
                    <>
                        <div className="relative max-w-2xl mx-auto">
                            <input
                                ...
                                className="w-full bg-white text-sm font-semibold text-[#212529] px-5 py-4 rounded-2xl border border-[#E9ECEF] focus:outline-none focus:ring-2 focus:ring-[#009EE2]/30 focus:border-[#009EE2] transition shadow-sm placeholder:text-[#ADB5BD]"
                            />
                        ...
                        <div className="max-w-2xl mx-auto space-y-3">
                            ...
                            {filteredUsers.length === 0 && (
                                <div className="text-center py-10">
                                    <p className="text-[#6C757D] font-medium text-sm">No users found.</p>
                                </div>
                            )}
                        </div>
                    </>
                    ...
                    <div className="max-w-2xl mx-auto space-y-8">
                                        ...
                                        return (
                                            <div key={req.senderId} className="flex items-center gap-4 p-4 bg-white border border-[#E9ECEF] rounded-2xl shadow-sm cursor-pointer" onClick={() => onNavigate(`public_profile_${req.senderId}`)}>
                                                ...
                                                <div className="shrink-0 flex gap-2" onClick={e => e.stopPropagation()}>
                                                    <button onClick={() => declinePartnerRequest(req.senderId)} className="text-xs font-bold text-[#6C757D] bg-neutral-100 hover:bg-neutral-200 px-4 py-2 rounded-xl transition">Decline</button>
                        ...
                        {receivedRequests.length === 0 && sentRequests.length === 0 && (
                            <div className="text-center py-10 bg-white border border-[#E9ECEF] rounded-3xl">
                                <p className="text-[#6C757D] font-medium text-sm">No pending requests.</p>
                            </div>
                        )}
                    </div>
                )}
