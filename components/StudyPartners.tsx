...
export const StudyPartners: React.FC<StudyPartnersProps> = ({ userProfile, onNavigate }) => {
    ...
    return (
        <div className="flex flex-col h-full w-full bg-[#F8F9FA] dark:bg-[#0A101F] animate-fade-in">
            {/* Header */}
            <div className="flex items-center px-4 py-4 bg-white dark:bg-[#121A2F] border-b border-[#E9ECEF] dark:border-white/10 shrink-0 sticky top-0 z-10">
                <button
                    onClick={() => window.dispatchEvent(new Event('app-go-back'))}
                    className="mr-3 w-10 h-10 flex items-center justify-center rounded-full hover:bg-neutral-100 text-[#6C757D] dark:text-[#A0ABC0] transition"
                >
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs font-semibold text-[#6C757D] dark:text-[#A0ABC0]">Build your academic network</p>
                </div>
            </div>

            {/* Tabs */}
            <div className="flex border-b border-[#E9ECEF] dark:border-white/10 bg-white dark:bg-[#121A2F] px-4 py-3 shrink-0 gap-3">
                <button
                    onClick={() => setActiveTab('find')}
                    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition ${activeTab === 'find' ? 'bg-[#009EE2] text-white shadow-md shadow-[#009EE2]/20' : 'text-[#6C757D] dark:text-[#A0ABC0] hover:text-[#212529] dark:text-white bg-neutral-50 hover:bg-neutral-100'}`}
                >
                    Find Partners
                </button>
                <button
                    onClick={() => setActiveTab('requests')}
                    className={`px-5 py-2.5 text-xs font-bold uppercase tracking-wider rounded-xl transition flex items-center gap-2 ${activeTab === 'requests' ? 'bg-[#009EE2] text-white shadow-md shadow-[#009EE2]/20' : 'text-[#6C757D] dark:text-[#A0ABC0] hover:text-[#212529] dark:text-white bg-neutral-50 hover:bg-neutral-100'}`}
                >
                    Requests
                    {(receivedRequests.length > 0 || sentRequests.length > 0) && (
                        <span className={`rounded-full text-[10px] font-black h-5 px-1.5 flex items-center justify-center ${activeTab === 'requests' ? 'bg-white dark:bg-[#121A2F] text-[#009EE2] dark:text-[#F8F9FA]' : 'bg-red-500 text-white'}`}>
                ...
                {activeTab === 'find' ? (
                    <>
                        <div className="relative max-w-2xl mx-auto">
                            <input
                                ...
                                className="w-full bg-white dark:bg-[#121A2F] text-sm font-semibold text-[#212529] dark:text-white px-5 py-4 rounded-2xl border border-[#E9ECEF] dark:border-white/10 focus:outline-none focus:ring-2 focus:ring-[#009EE2]/30 focus:border-[#009EE2] transition shadow-sm placeholder:text-[#ADB5BD]"
                            />
                        ...
                        <div className="max-w-2xl mx-auto space-y-3">
                            {filteredUsers.map(u => {
                                ...
                                return (
                                    <div key={u.uid} className="flex items-center gap-4 p-4 bg-white dark:bg-[#121A2F] border border-[#E9ECEF] dark:border-white/10 rounded-2xl shadow-sm hover:shadow-md transition cursor-pointer" onClick={() => onNavigate(`public_profile_${u.uid}`)}>
                                        ...
                                        <div className="min-w-0 flex-1">
                                            ...
                                            {u.department_id && <p className="text-xs text-[#6C757D] dark:text-[#A0ABC0] font-medium truncate mt-0.5">{u.department_id.replace(/_/g, ' ')}</p>}
                                        </div>
                            ...
                            {filteredUsers.length === 0 && (
                                <div className="text-center py-10">
                                    <p className="text-[#6C757D] dark:text-[#A0ABC0] font-medium text-sm">No users found.</p>
                                </div>
                            )}
                        </div>
                    </>
                    ...
                    <div className="max-w-2xl mx-auto space-y-8">
                        {receivedRequests.length > 0 && (
                            <div>
                                <h3 className="text-xs font-black uppercase tracking-wider text-[#6C757D] dark:text-[#A0ABC0] mb-4 pl-1">Received Requests ({receivedRequests.length})</h3>
                                <div className="space-y-3">
                                    {receivedRequests.map((req: any) => {
                                        const sender = allUsers.find(u => u.uid === req.senderId) || missingProfiles[req.senderId];
                                        return (
                                            <div key={req.senderId} className="flex items-center gap-4 p-4 bg-white dark:bg-[#121A2F] border border-[#E9ECEF] dark:border-white/10 rounded-2xl shadow-sm cursor-pointer" onClick={() => onNavigate(`public_profile_${req.senderId}`)}>
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <p className="text-xs text-[#6C757D] dark:text-[#A0ABC0] font-medium truncate mt-0.5">Wants to connect with you</p>
                                                </div>
                                                <div className="shrink-0 flex gap-2" onClick={e => e.stopPropagation()}>
                                                    <button onClick={() => declinePartnerRequest(req.senderId)} className="text-xs font-bold text-[#6C757D] dark:text-[#A0ABC0] bg-neutral-100 hover:bg-neutral-200 px-4 py-2 rounded-xl transition">Decline</button>
                        ...
                        {sentRequests.length > 0 && (
                            <div>
                                <h3 className="text-xs font-black uppercase tracking-wider text-[#6C757D] dark:text-[#A0ABC0] mb-4 pl-1">Sent Requests ({sentRequests.length})</h3>
                                <div className="space-y-3">
                                    {sentRequests.map((req: any) => {
                                        const receiver = allUsers.find(u => u.uid === req.receiverId) || missingProfiles[req.receiverId];
                                        return (
                                            <div key={req.receiverId} className="flex items-center gap-4 p-4 bg-white dark:bg-[#121A2F] border border-[#E9ECEF] dark:border-white/10 rounded-2xl shadow-sm cursor-pointer hover:shadow-md transition" onClick={() => onNavigate(`public_profile_${req.receiverId}`)}>
                        ...
                        {receivedRequests.length === 0 && sentRequests.length === 0 && (
                            <div className="text-center py-10 bg-white dark:bg-[#121A2F] border border-[#E9ECEF] dark:border-white/10 rounded-3xl">
                                <p className="text-[#6C757D] dark:text-[#A0ABC0] font-medium text-sm">No pending requests.</p>
                            </div>
                        )}
                    </div>
                )}
