...
export function formatLatexMath(content: string | null | undefined): string {
    ...
    // If it's already a full block equation $$...$$, clean inner whitespace and return
    ...
    // 1. Convert units with powers (e.g. m/s^2, m/s², kg*m/s^2, cm^3, m^2)
    ...
    // 2. Convert standard scientific powers of 10 (e.g. 10^5, 10^-3, 10^12)
    text = text.replace(/\b10\^([+-]?\d+)\b/g, (_m, p) => `$10^{${p}}$`);

    // 3. Convert square roots: sqrt(x) or sqrt(2gh) or sqrt(a^2 + b^2)
    text = text.replace(/\bsqrt\(([^)]+)\)/gi, (_m, inner) => `$\\sqrt{${inner.trim()}}$`);

    // 4. Convert Greek letter names to LaTeX when in formula-like expressions
    text = text.replace(/\b(theta|alpha|beta|lambda|omega|delta|mu|phi|gamma|pi)\b(?=[^a-zA-Z]|$)/gi, (match) => {
        const name = match.toLowerCase();
        return `$\\${name}$`;
    });

    // 5. Convert commonly subscripted physics/math variables: v_i, v_f, v_0, a_x, F_net, F_N, F_g, f_k, t_0, x_0
    ...
    // 6. Convert variables/numbers raised to power: x^2, t^2, v^2, r^3, (v_f)^2, etc.
    ...
    // 7. Convert degree expressions: 30° or 30 deg or 45 degrees
    text = text.replace(/\b(\d+)\s*(?:°|deg|degrees)\b/gi, (_m, deg) => `$${deg}^\\circ$`);

    // 8. Convert +/- symbol to \pm
    ...
    // Clean up any accidentally doubled dollar signs like $$x^2$$ within inline sentences
    text = text.replace(/\$\$([^\$\n]+?)\$\$/g, (match, inner) => {
        // If it's a short inline term, keep single $, otherwise keep $$
        if (inner.length < 25 && !inner.includes('\\begin') && !inner.includes('\n')) {
            return `$${inner.trim()}$`;
        }
        return match;
    });
